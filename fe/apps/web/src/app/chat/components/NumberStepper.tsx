...
export default function NumberStepper({
  ...
  return (
    <div
      className={`flex items-center justify-between ${className} w-24 sm:w-32 md:w-24`}
      role="spinbutton"
      aria-valuenow={value}
      aria-valuemin={min}
      aria-valuemax={max}
    ...
    >
      <button
        ...
        onClick={decrement}
        disabled={value <= min}
        className={`relative z-0 inline-flex items-center justify-center rounded-full border px-1 py-1 text-xs transition-colors disabled:opacity-40 disabled:pointer-events-none focus:outline-none cursor-pointer`}
      >
        <MinusNumberIcon aria-hidden />
      </button>
